...
var FF3 = (function (window, $, module, undefined) {
  ...
  function randomizeMonsterPalettes(ROM) {
    for (var i = 0; i < 255; i++) {
      ROM[module.address.encounterLists + i * 6] = parseInt(
        Math.random() * 254 + 1
      );
      ROM[module.address.encounterLists + i * 6 + 1] = parseInt(
        Math.random() * 254 + 1
      );
    }
  }
  ...
  function randomizeWeaponAnimations(ROM) {
    for (var i = 0; i < 86; i++) {
      ...
      ROM[module.address.weaponGFX + i * 3 + 1] =
        module.weapon_sprites[
          parseInt(Math.random() * module.weapon_sprites.length)
        ];
    }
  }
  ...
  function randomizeWeaponPalettes(ROM) {
    for (var i = 0; i < 86; i++) {
      ROM[module.address.weaponGFX + i * 3 + 2] =
        module.weapon_palettes[
          parseInt(Math.random() * module.weapon_palettes.length)
        ];
    }
  }
