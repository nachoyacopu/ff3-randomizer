...
var FF3 = (function(window, $, module, undefined) {
    ...
    module.Job.prototype.loadFromROM = function(ROM, id) {
        this._id = id;
        this.commands = ROM.slice(module.address.jobCommands+(id*4), module.address.jobCommands+(id*4)+4);
        this.base = ROM.slice(module.address.jobBaseData+(id*8), module.address.jobBaseData+(id*8)+8);
        this.growth = ROM.slice(module.address.jobGrowth+(id*196), module.address.jobGrowth+(id*196)+196);
        this.namePtr = ROM.slice(module.address.jobNamePtr+(id*2), module.address.jobNamePtr+(id*2)+2);
        this.data1 = ROM.slice(module.address.jobData1+(id*5), module.address.jobData1+(id*5)+5);
        this.paletteOverworld = ROM.slice(module.address.palettesOverworld+(id*4), module.address.palettesOverworld+(id*4)+4);
        this.spritesOverworld = ROM.slice(module.address.spritesOverworld+(id*256), module.address.spritesOverworld+(id*256)+256);
        this.paletteIndexBattle = ROM.slice(module.address.paletteIndexBattle+id+4, module.address.paletteIndexBattle+id+5);
        this.spritesBattle = ROM.slice(module.address.spritesBattle+(id*672), module.address.spritesBattle+(id*672)+672);
    ...
    module.Job.prototype.saveToROM = function(ROM, id) {
        ...
        ROM.set(this.paletteIndexBattle, module.address.paletteIndexBattle+id+4);
        ...
        for(var i=0;i<64;i++) {
            setEquipability(ROM, id, i, this.equipabilitySet[i >> 3] & (1 << (i & 7)));
        };
    ...
    function getEquipability(ROM, jobId, equipSet) {
        return ((ROM[module.address.equipabilitySets+(equipSet*3)+(2-(jobId >> 3))] & (1 << (jobId & 7))) !== 0)?1:0;
    };
    
    function setEquipability(ROM, jobId, equipSet, value) {
        if (value === 0) {
            ROM[module.address.equipabilitySets+(equipSet*3)+(2-(jobId >> 3))] &= ~(1 << (jobId & 7));
        } else {
            ROM[module.address.equipabilitySets+(equipSet*3)+(2-(jobId >> 3))] |= (1 << (jobId & 7));
        };
    };
